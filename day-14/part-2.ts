...
while (cave.get(currentSand[0])?.get(currentSand[1]) !== "o") {
  ...
  if ((!down || down === ".") && currentSand[1] < caveMaxY + 1) {
    currentSand[1]++;
    continue;
  }

  if ((!downLeft || downLeft === ".") && currentSand[1] < caveMaxY + 1) {
    currentSand[0]--;
    currentSand[1]++;
    continue;
  }

  if ((!downRight || downRight === ".") && currentSand[1] < caveMaxY + 1) {
    currentSand[0]++;
    currentSand[1]++;
    continue;
  }
